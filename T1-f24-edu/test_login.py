...
class TestLogin(unittest.TestCase):
    ...
    def test_valid_login(self):
        response = self.app.post(url_for('login'), data={'username': 'testuser', 'password': 'testpassword'}, follow_redirects=True)
        self.assertIn(b'Pick a Path', response.data) # Check for successful redirect

    def test_invalid_username(self):
        response = self.app.post(url_for('login'), data={'username': 'invaliduser', 'password': 'testpassword'}, follow_redirects=True)
        self.assertIn(b'Invalid username or password', response.data)
